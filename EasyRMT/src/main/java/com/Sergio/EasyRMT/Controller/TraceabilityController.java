...
public class TraceabilityController {
    ...
    @RequestMapping(value =PATH_BASE+"{projectId}/obj1/{obj1Id}", method = RequestMethod.POST)
    public ResponseEntity createRelationship(@PathVariable int projectId, @PathVariable int obj1Id, @ModelAttribute TraceDom traces,
                                             Principal principal){
        ProjectDom project = projectService.getProject(projectId);
        UserDom user = userService.findUser(principal.getName());
        List<ProjectDom> projectDomList = commonMethods.getProjectsFromGroup(user);
        if (commonMethods.isAllowed(projectDomList, project)) {
            boolean created = traceabilityService.saveRelationship(obj1Id,traces);
            if (created) {
                return ResponseEntity.status(HttpStatus.OK).body("");
            } else {
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("");
        ...
        LOGGER.log(Level.INFO, loggerMessage+"User "+principal.getName()+" has tried to create a traceability " +
                "relationship from project "+projectId);
        throw new AccessDeniedException("Not allowed");
    }
